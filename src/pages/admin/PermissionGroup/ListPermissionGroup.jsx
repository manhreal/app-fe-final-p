...
import { actionGetListPermissionGroups, actionDeletePermissionGroup } from '../../../redux/permission_group/actions';
import ActionEditDelete from '../../../components/ActionEditDelete';
import SimplePagination from '../../../components/Pagination/Pagination';
import CreateUpdatePermissionGroup from './CreateUpdatePermissionGroup';
...
const ListPermissionGroups = () => {
    ...
    const { dataListPermissionGroups, loading } = useSelector(state => state.permissionGroups);
    console.log(dataListPermissionGroups);
    ...
    const handleSearch = useCallback(async (resetPage = true) => {
        ...
        try {
            const result = await dispatch(actionGetListPermissionGroups(params));
    ...
    const handlePageChange = useCallback((page) => {
        ...
        dispatch(actionGetListPermissionGroups(params));
    }, [dispatch, form, pagination, buildSearchParams]);
    ...
    const handleReset = useCallback(async () => {
        ...
        try {
            await dispatch(actionGetListPermissionGroups(params));
    ...
    const handleView = useCallback((record) => {
        console.log('View permission group:', record);
        navigate(`/admin/permission-groups/info-permission-group/${record.id}`);
    }, [navigate]);
    ...
    const handleDelete = useCallback(async (record) => {
        ...
        const result = await showDeleteConfirmDialog(record.name, 'nhóm quyền');

        if (!result.isConfirmed) return;

        try {
            const deleteResult = await dispatch(actionDeletePermissionGroup(record.id));

            if (deleteResult?.success || deleteResult?.status === 200 || deleteResult) {
                // Show success alert
                await showSuccessToast('Xóa nhóm quyền thành công');

                handleSearch(true);
            } else {
                // Show error alert
                await showErrorAlert(deleteResult?.message || 'Xóa nhóm quyền thất bại');
            }
        } catch (error) {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            render: (permissions) => (
                <Tag color="green">
                    {permissions ? permissions.length : 0} quyền
                </Tag>
            )
        },
        {
                    ...
                    >
                        <Row gutter={16}>
                            <Col xs={24} sm={12} md={8}>
                                <Form.Item label="Tên nhóm quyền" name="name">
                                    <Input
                            ...
                            <Col xs={24} sm={12} md={8}>
                                <Form.Item label="Mã nhóm quyền" name="code">
                                    <Input
                ...
                <Card
                    ...
                    extra={
                        <span style={{ color: '#666' }}>
                            Tổng số: {pagination.total} nhóm quyền
                        </span>
                    }
                    className="w-full"
                >
                    <div className="w-full overflow-x-auto -mx-3 sm:mx-0 max-w-full">
                        <div className="px-3 sm:px-0">
                            <Table
                                columns={columns}
                                dataSource={dataListPermissionGroups?.rows || []}
                                loading={loading}
                                rowKey="id"
                                pagination={false}
                                size="middle"
