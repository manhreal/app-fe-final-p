...
import {
    actionGetDetailPermissionGroup,
    actionCreatePermissionGroup,
    actionUpdatePermissionGroup
} from '../../../redux/permission_group/actions';
import {
    showErrorAlert,
    showSuccessToast
...
const CreateUpdatePermissionGroup = ({
    visible,
    onCancel,
    onSuccess,
    editId = null,
    ...
    const loadDetailData = async () => {
        ...
        try {
            const result = await dispatch(actionGetDetailPermissionGroup(editId));
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            if (isEditMode) {
                result = await dispatch(actionUpdatePermissionGroup(editId, values));
            } else {
                result = await dispatch(actionCreatePermissionGroup(values));
            }

            if (result?.success || result?.status === 200 || result) {
                // Show success alert
                await showSuccessToast(`${isEditMode ? 'Cập nhật' : 'Tạo mới'} nhóm quyền thành công`);

                form.resetFields();
                onSuccess && onSuccess();
            } else {
                // Show error alert
                await showErrorAlert(result?.message || `${isEditMode ? 'Cập nhật' : 'Tạo mới'} nhóm quyền thất bại`);
            }
        } catch (error) {
    ...
    return (
        <Modal
            title={isEditMode ? 'Sửa nhóm quyền' : 'Thêm mới nhóm quyền'}
            open={visible}
            ...
            >
                <Form.Item
                    ...
                    rules={[
                        ...
                        { max: 255, message: 'Tên nhóm quyền không được vượt quá 255 ký tự' }
                    ]}
                >
                    <Input
                ...
                <Form.Item
                    ...
                    rules={[
                        ...
                        { max: 100, message: 'Mã nhóm quyền không được vượt quá 100 ký tự' },
                        {
                            pattern: /^[a-z0-9_]+$/,
...
export default CreateUpdatePermissionGroup;