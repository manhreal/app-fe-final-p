...
import { actionGetListExams, actionGetDetailExam, actionRemoveExamFromEvent } from '../../../redux/exam/actions';
...
import CreateUpdateExam from './CreateUpdateExam';
...
const ListExams = ({ examEventId }) => {
    ...
    const handleEdit = useCallback(async (record) => {
        ...
        try {
            await dispatch(actionGetDetailExam(record.id));
            setModalMode('edit');
            setEditId(record.id);
            setModalVisible(true);
        } catch (error) {
            console.error('Error getting exam detail:', error);
            await showErrorAlert('Đã xảy ra lỗi khi tải thông tin đề thi. Vui lòng thử lại.');
        }
    }, [dispatch]);

    const handleRemoveFromEvent = useCallback(async (record) => {
        const result = await showDeleteConfirmDialog(
            record.name, 
            'đề thi khỏi kỳ thi này', 
            'Đề thi sẽ được giữ lại nhưng không còn thuộc kỳ thi này'
        );

        if (!result.isConfirmed) return;

        try {
            const removeResult = await dispatch(actionRemoveExamFromEvent(record.id, examEventId));

            if (removeResult?.success || removeResult?.status === 200 || removeResult) {
                await showSuccessToast('Đã xóa đề thi khỏi kỳ thi');
                handleSearch(true);
            } else {
                await showErrorAlert(removeResult?.message || 'Xóa đề thi khỏi kỳ thi thất bại');
            }
        } catch (error) {
            console.error('Remove from event error:', error);
            await showErrorAlert('Đã xảy ra lỗi, vui lòng thử lại');
        }
    }, [dispatch, examEventId, handleSearch]);
        ...
        },
        {
            ...
            render: (_, record) => (
                <ActionEditDelete
                    ...
                    onclickDelete={() => handleRemoveFromEvent(record)}
                    tooltipText={record.name}
                    hideView={true}
                />
            )
        }
    ], [pagination, handleEdit, handleRemoveFromEvent]);
