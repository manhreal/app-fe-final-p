...
import { actionGetListClassUsers, actionDeleteClassUser, actionGetDetailClassUser } from '../../../redux/class_user/actions';
...
import CreateClassUser from './CreateClassUser';
import {
    showErrorAlert,
    showSuccessToast,
...
const ROLE_OPTIONS = [
    { value: '3', label: 'Học sinh', color: 'blue' },
    { value: '7', label: 'Giáo viên', color: 'purple' }
];
...
const ListClassUsers = ({ classId }) => {
    ...
    const handleEdit = useCallback(async (record) => {
        console.log('Edit class user:', record);
        try {
            await dispatch(actionGetDetailClassUser(record.id));
            setModalMode('edit');
            setEditId(record.id);
            setModalVisible(true);
        } catch (error) {
            console.error('Error getting class user detail:', error);
            await showErrorAlert('Đã xảy ra lỗi khi tải thông tin người dùng. Vui lòng thử lại.');
        }
    }, [dispatch]);
    ...
    const handleDelete = useCallback(async (record) => {
        ...
        try {
            const deleteResult = await dispatch(actionDeleteClassUser(record.id));

            if (deleteResult?.success || deleteResult?.status === 200 || deleteResult) {
                await showSuccessToast('Xóa người dùng khỏi lớp học thành công');

                handleSearch(true);
            } else {
                await showErrorAlert(deleteResult?.message || 'Xóa người dùng khỏi lớp học thất bại');
            }
        } catch (error) {
    ...
    const handleAddNew = useCallback(() => {
        console.log('Add new class user');
        setModalMode('create');
        setEditId(null);
        setModalVisible(true);
    }, []);
