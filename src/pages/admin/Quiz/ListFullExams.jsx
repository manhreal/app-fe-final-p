...
import { SearchOutlined, PlusOutlined, ReloadOutlined, UserOutlined, FileTextOutlined } from '@ant-design/icons';
import { actionGetListExams, actionDeleteExam, actionGetDetailExam } from '../../../redux/exam/actions';
...
import CreateUpdateExam from '../ExamEvent/CreateUpdateExam';
...
const ListFullExams = () => {
    ...
    const handleDelete = useCallback(async (record) => {
        const result = await showDeleteConfirmDialog(record.name, 'đề thi');

        if (!result.isConfirmed) return;

        try {
            const deleteResult = await dispatch(actionDeleteExam(record.id));

            if (deleteResult?.success || deleteResult?.status === 200 || deleteResult) {
                await showSuccessToast('Xóa đề thi thành công');

                handleSearch(true);
            } else {
                await showErrorAlert(deleteResult?.message || 'Xóa đề thi thất bại');
            }
        } catch (error) {
