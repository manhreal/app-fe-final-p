...
import { actionGetListQuizDifficulties, actionDeleteQuizDifficulty } from '../../../redux/quiz_difficulty/actions';
import ActionEditDelete from '../../../components/ActionEditDelete';
import SimplePagination from '../../../components/Pagination/Pagination';
import CreateUpdateQuizDifficulty from './CreateUpdateQuizDifficulty';
import {
    showErrorAlert,
    showSuccessToast,
...
const ListQuizDifficulties = () => {
    const dispatch = useDispatch();
    const { dataListQuizDifficulties, loading } = useSelector(state => state.quizDifficulties);
    console.log(dataListQuizDifficulties);
    ...
    const buildSearchParams = useCallback((formValues = {}, paginationInfo = pagination) => {
        const { level, name, description } = formValues;
        return {
            ...(level && { level }),
            ...(name && { name }),
    ...
    const handleSearch = useCallback(async (resetPage = true) => {
        ...
        try {
            const result = await dispatch(actionGetListQuizDifficulties(params));
            ...
            console.error('Error fetching quiz difficulties:', error);
    ...
    const handlePageChange = useCallback((page) => {
        ...
        dispatch(actionGetListQuizDifficulties(params));
    }, [dispatch, form, pagination, buildSearchParams]);
    ...
    const handleReset = useCallback(async () => {
        ...
        try {
            await dispatch(actionGetListQuizDifficulties(params));
    ...
    const handleEdit = useCallback((record) => {
        console.log('Edit quiz difficulty:', record);
        setModalMode('edit');
        setEditId(record.id);
    ...
    const handleDelete = useCallback(async (record) => {
        ...
        const result = await showDeleteConfirmDialog(record.name, 'độ khó');

        if (!result.isConfirmed) return;

        try {
            const deleteResult = await dispatch(actionDeleteQuizDifficulty(record.id));

            if (deleteResult?.success || deleteResult?.status === 200 || deleteResult) {
                // Show success alert
                await showSuccessToast('Xóa độ khó câu hỏi thành công');

                handleSearch(true);
            } else {
                // Show error alert
                await showErrorAlert(deleteResult?.message || 'Xóa độ khó câu hỏi thất bại');
            }
        } catch (error) {
    ...
    const handleAddNew = useCallback(() => {
        console.log('Add new quiz difficulty');
        setModalMode('create');
        setEditId(null);
        setModalVisible(true);
    }, []);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            width: 150,
            render: (level) => (
                <div title={level} className="max-w-40 text-red-500 font-medium">
                    {truncateText(level, 50)}
                </div>
            )
        },
        {
        ...
        <div className="w-full max-w-full min-h-screen">
            <div className="mx-auto max-w-full lg:max-w-7xl">
               {/* Search Form */}
                    ...
                    >
                        <Row gutter={16}>
                            <Col xs={24} sm={12} md={8}>
                                <Form.Item label="Cấp độ khó câu hỏi" name="level">
                                    <Input
                                        placeholder="Nhập cấp độ khó câu hỏi"
                                        allowClear
                                    />
                                </Form.Item>
                            </Col>
                            <Col xs={24} sm={12} md={8}>
                                <Form.Item label="Tên độ khó câu hỏi" name="name">
                                    <Input
                                        placeholder="Nhập tên độ khó câu hỏi"
                                        allowClear
                                    />
                                </Form.Item>
                            </Col>
                ...
                {/* Data Table */}
                <Card
                    title="Danh sách độ khó câu hỏi"
                    extra={
                        <span style={{ color: '#666' }}>
                            Tổng số: {pagination.total} độ khó
                        </span>
                    }
                    className="w-full"
                >
                    <div className="w-full overflow-x-auto -mx-3 sm:mx-0 max-w-full">
                        <div className="px-3 sm:px-0">
                            <Table
                                columns={columns}
                                dataSource={dataListQuizDifficulties?.rows || []}
                                loading={loading}
                                rowKey="id"
                                pagination={false}
                                size="middle"
...
export default ListQuizDifficulties;