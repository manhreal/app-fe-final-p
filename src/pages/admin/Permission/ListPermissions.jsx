...
import { actionGetListPermissions, actionDeletePermission } from '../../../redux/permission/actions';
...
const ListPermissions = () => {
    ...
    const { dataListPermissions, loading } = useSelector(state => state.permissions);
    console.log(dataListPermissions);
    ...
    const buildSearchParams = useCallback((formValues = {}, paginationInfo = pagination) => {
        const { name, code, description, group_id } = formValues;
        return {
            ...(name && { name }),
            ...(code && { code }),
    ...
    const handleSearch = useCallback(async (resetPage = true) => {
        ...
        try {
            const result = await dispatch(actionGetListPermissions(params));
    ...
    const handlePageChange = useCallback((page) => {
        ...
        dispatch(actionGetListPermissions(params));
    }, [dispatch, form, pagination, buildSearchParams]);
    ...
    const handleReset = useCallback(async () => {
        ...
        try {
            await dispatch(actionGetListPermissions(params));
    ...
    const handleView = useCallback((record) => {
        console.log('View permission:', record);
        navigate(`/admin/permissions/info-permission/${record.id}`);
    }, [navigate]);
    ...
    const handleDelete = useCallback(async (record) => {
        ...
        const result = await showDeleteConfirmDialog(record.name, 'quyền');

        if (!result.isConfirmed) return;

        try {
            const deleteResult = await dispatch(actionDeletePermission(record.id));

            if (deleteResult?.success || deleteResult?.status === 200 || deleteResult) {
                // Show success alert
                await showSuccessToast('Xóa quyền thành công');

                handleSearch(true);
            } else {
                // Show error alert
                await showErrorAlert(deleteResult?.message || 'Xóa quyền thất bại');
            }
        } catch (error) {
                ...
                <Card style={{ marginBottom: '16px' }} className="w-full">
                    ...
                    >
                        <Row gutter={16}>
                            ...
                            <Col xs={24} sm={12} md={6}>
                                <Form.Item label="Mã quyền" name="code">
                                    <Input
                            ...
                            <Col xs={24} sm={12} md={6}>
                                <Form.Item label="ID nhóm quyền" name="group_id">
                                    <Input
                                        placeholder="Nhập ID nhóm quyền"
                                        allowClear
                                        type="number"
                                    />
                                </Form.Item>
                ...
                <Card
                    ...
                    extra={
                        <span style={{ color: '#666' }}>
                            Tổng số: {pagination.total} quyền
                        </span>
                    }
                    className="w-full"
                >
                    <div className="w-full overflow-x-auto -mx-3 sm:mx-0 max-w-full">
                        <div className="px-3 sm:px-0">
                            <Table
                                columns={columns}
                                dataSource={dataListPermissions?.rows || []}
                                loading={loading}
                                rowKey="id"
                                pagination={false}
                                size="middle"
