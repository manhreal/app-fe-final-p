...
import MenuList from '../../components/math_tool/MenuList';
import ContentMenu from '../../components/math_tool/ContentMenu';

const MathTool = () => {
    const [collapsed, setCollapsed] = useState(false);
    // const [activeMenuName, setActiveMenuName] = useState('Hướng dẫn sử dụng');
    ...
    useEffect(() => {
        const checkMobile = () => {
            ...
            // Nếu chuyển từ mobile sang desktop, đóng mobile menu
            if (!mobile && mobileMenuOpen) {
                setMobileMenuOpen(false);
            }

            // Nếu chuyển từ desktop sang mobile, reset collapsed state
            if (mobile) {
                setCollapsed(false);
            }
        };
    ...
    // Đóng mobile menu khi chọn menu item
    ...
    // Xử lý ESC key để đóng mobile menu
    useEffect(() => {
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            {/* Mobile Toggle Button - Fixed position bottom right, ẩn khi menu mở */}
            {isMobile && !mobileMenuOpen && (
                <button
